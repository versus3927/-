...
17. If a player is registered as `0 0 13` because the roster nickname does not match, the player is absent, or a visible scoreboard row contains `0/0/0`, `WARN_CHANNEL_ID` receives one separate warning message per player. The first line tags the Discord user, the second line is `Додж статистики - #MATCH` for a wrong nickname/zero row or `Отсутствие на финальном скриншоте - #MATCH` for an absent row, and the original screenshot is attached to the same message. Members with the `🔴 Pro League` role or an ID listed in the code are skipped. The same warning message with its screenshot is copied to `LOG_CHANNEL_ID`.
