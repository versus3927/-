...
10. A scoreboard is accepted when at least six players in total are reliably matched by nickname, with at least one match on each side. Each unmatched card player is registered as `0 0 13`; every matched player keeps the statistics read from the screenshot.
