...
6. For every `на проверку` card the bot first clicks `Получить игроков` and reads the authoritative short IDs and starting `# CT`/`# T` groups from the private helper response. The helper intentionally leaves `<счёт A> <счёт B>` placeholders, so the bot separately reads the real final score from the result card or attached screenshot and inserts both numbers into its own `=g` command. It checks all ten K/A/D rows when they are present in the card. If anything is missing or inconsistent, the match is skipped instead of guessed. Complete `внесён` cards keep the deterministic/AI fallback flow. The source card is deleted and counted only after the registration bot replies `Готово`; on `Не вышло` or timeout it is preserved for retry.
